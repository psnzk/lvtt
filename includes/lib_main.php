...
function assign_ur_here($cat = 0, $str = '', $strArr = array(), $url = '', $ru_id = 0)
{
	...
	if ($filename != 'index') {
		if (in_array($filename, array('category', 'goods', 'article_cat', 'article', 'brand', 'single_sun', 'store_street', 'presale'))) {
			if (('category' == $filename) || ('goods' == $filename) || ('brand' == $filename) || ('single_sun' == $filename) || ('presale' == $filename)) {
				if (0 < $cat) {
					if ($filename == 'presale') {
			...
			$page_title = $GLOBALS['_LANG']['shopping_activity'] . '_' . $page_title;
			$args = array('auid' => '0');
			$ur_here .= '<span class="arrow">></span>' . '<span> <a href="presale.php">' . '预售' . '</a>' . '</span>';
		}
		else if ('gift_gard' == $filename) {
			$page_title = '礼品卡兑换' . '_' . $page_title;
			$args = array('wsid' => '0');
			$ur_here .= ' <code>&gt;</code> <a href="gift_gard.php">' . '礼品卡兑换' . '</a>';
		}
		else if ('topic' == $filename) {
			$page_title = '专题' . '_' . $page_title;
			$args = array('id' => '0');
			$ur_here .= '<span class="arrow">></span>' . '<span> <a href="topic.php">' . '专题' . '</a>' . '</span>';
		}
	}

	if (!empty($str)) {
		$filename_arr = array('group_buy', 'auction', 'store_street', 'snatch', 'wholesale', 'exchange', 'single_sun', 'activity', 'presale', 'gift_gard', 'topic');
	...
	while (1) {
		foreach ($arr as $row) {
			if ($cat == $row['cat_id']) {
				...
				$sql = 'SELECT c.cat_id, c.cat_name FROM ' . $GLOBALS['ecs']->table('category') . ' AS c WHERE parent_id = \'' . $row['cat_id'] . '\' ' . 'AND (SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('merchants_category') . ' AS mc WHERE mc.cat_id = c.cat_id LIMIT 1) = 0';
...
function assign_pager($app, $cat, $record_count, $size, $sort, $order, $page = 1, $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr = '', $url_format = '', $sch_array = '', $merchant_id = 0, $keyword = '', $ubrand = '', $act = '', $ship = '', $self = '', $mbid = 0)
{
