...
function build_uri($app, $params, $append = '', $page = 0, $keywords = '', $size = 0)
{
	...
	$args = array('cid' => 0, 'gid' => 0, 'bid' => 0, 'acid' => 0, 'aid' => 0, 'mid' => 0, 'urid' => 0, 'ubrand' => 0, 'chkw' => '', 'is_ship' => '', 'hid' => 0, 'sid' => 0, 'gbid' => 0, 'auid' => 0, 'sort' => '', 'order' => '', 'status' => -1);
...
function get_store_shop_list($libType = 0, $keywords = '', $count = 0, $size = 15, $page = 1, $sort = 'shop_id', $order = 'DESC', $warehouse_id = 0, $area_id = 0, $store_province = 0, $store_city = 0, $store_district = 0, $store_user = '')
{
	...
	$sql = 'SELECT msi.shop_id, msi.user_id, msi.shoprz_brandName, msi.shopNameSuffix ' . $select . ' FROM ' . $GLOBALS['ecs']->table('merchants_shop_information') . ' as msi LEFT JOIN ' . $GLOBALS['ecs']->table('merchants_grade') . ' AS mg ON mg.ru_id = msi.user_id ' . $where_table . ' where ' . $whereShop . ' AND msi.merchants_audit = 1 ORDER BY ' . $sort . ' ' . $order . ' ' . $limit;
	$res = $GLOBALS['db']->query($sql);
	$arr = array();
...
{
	$sql = 'SELECT cat_id, cat_name, category_links, cat_alias_name FROM ' . $GLOBALS['ecs']->table('category') . ' WHERE parent_id = 0 AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC limit 16';
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
...
function get_category_list($cat_id = 0, $relation = 0, $seller_shop_cat = array(), $user_id = 0, $for_level = 0, $table = 'category')
{
	...
	$category_list = $GLOBALS['db']->getAll(' SELECT cat_id, cat_name FROM ' . $GLOBALS['ecs']->table($table) . ' WHERE parent_id = \'' . $parent_id . '\' ' . $where);
...
function get_goods_activity_info($act_id = 0, $select = array())
{
	...
	$sql = 'SELECT ' . $select . ' FROM ' . $GLOBALS['ecs']->table('goods_activity') . ' WHERE act_id = \'' . $act_id . '\'';
