...
{
	$sql = 'SELECT og.rec_id, og.goods_id, og.ru_id, og.goods_name, og.goods_sn, og.market_price, og.goods_number, og.warehouse_id, ' . 'og.goods_price, og.goods_attr, og.is_real, og.parent_id, og.is_gift, ' . 'og.goods_price * og.goods_number AS subtotal, og.extension_code,g.shop_price, g.is_shipping, g.goods_weight AS goodsWeight ' . 'FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON og.goods_id = g.goods_id ' . ' WHERE og.order_id = \'' . $order_id . '\'';
	$res = $GLOBALS['db']->query($sql);
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0, $warehouse_id = 0, $area_id = 0, $stages_qishu = '-1', $store_id = 0)
{
	$GLOBALS['err']->clean();
	$_parent_id = $parent;
...
function user_bonus($user_id, $goods_amount = 0, $cart_value = 0)
{
	...
	$sql = 'SELECT t.type_id, t.type_name, t.type_money, b.bonus_id ' . 'FROM ' . $GLOBALS['ecs']->table('bonus_type') . ' AS t,' . $GLOBALS['ecs']->table('user_bonus') . ' AS b ' . 'WHERE t.type_id = b.bonus_type_id ' . 'AND t.use_start_date <= \'' . $today . '\' ' . 'AND t.use_end_date >= \'' . $today . '\' ' . 'AND t.min_goods_amount <= \'' . $goods_amount . '\' ' . 'AND b.user_id<>0 ' . 'AND b.user_id = \'' . $user_id . '\' ' . 'AND b.order_id = 0' . $where;
	return $GLOBALS['db']->getAll($sql);
}
...
function get_cart_goods($cart_value = '', $type = 0, $warehouse_id = 0, $area_id = 0)
{
	...
	$sql = 'SELECT c.*, IF(c.parent_id, c.parent_id, c.goods_id) AS pid ' . ' FROM ' . $GLOBALS['ecs']->table('cart') . ' AS c ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON c.goods_id = g.goods_id ' . ' WHERE ' . $sess_id . ' AND c.rec_type = \'' . CART_GENERAL_GOODS . '\' AND c.stages_qishu =\'-1\'' . $goodsIn . $where . ' ORDER BY c.rec_id DESC';
	$res = $GLOBALS['db']->query($sql);
...
function get_buy_cart_goods_number($type = CART_GENERAL_GOODS, $cart_value = '', $ru_type = 0)
{
	...
	$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('cart') . ' AS c LEFT JOIN ' . $GLOBALS['ecs']->table('goods_activity') . ' AS ga ON c.goods_id = ga.act_id WHERE ' . $c_sess . 'AND rec_type = \'' . $type . '\'' . $goodsIn . ' AND c.extension_code = \'package_buy\'';
...
function check_consignee_info($consignee, $flow_type)
{
	if (exist_real_goods(0, $flow_type)) {
		...
		if ($res) {
			...
			else if (empty($consignee['district'])) {
...
function order_query_sql($type = 'finished', $alias = '')
{
	...
	if ($type == 'confirm_take') {
		return ' AND ' . $alias . 'order_status ' . db_create_in(array(OS_CONFIRMED, OS_SPLITED)) . ' AND ' . $alias . 'shipping_status ' . db_create_in(array(SS_RECEIVED)) . ' AND ' . $alias . 'pay_status ' . db_create_in(array(PS_PAYED)) . ' ';
	}
...
{
	return '   ' . $alias . 'goods_amount + ' . $alias . 'tax + ' . $alias . 'shipping_fee' . ' + ' . $alias . 'insure_fee + ' . $alias . 'pay_fee + ' . $alias . 'pack_fee' . ' + ' . $alias . 'card_fee -' . $alias . 'discount ';
}
...
function get_return_refound($order_id, $rec_id, $num)
{
	...
	if (1 < $count_goods) {
		$return_ratio = $aount_price / ($goods_price + $orders['shipping_fee']);
...
{
	$sql = 'SELECT (' . order_commission_field('o.') . ') AS total_fee ' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('order_goods') . ' AS og ON o.order_id=og.order_id WHERE o.order_id = \'' . $order_id . '\'';
