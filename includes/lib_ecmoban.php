	...
	if ($row) {
		if ($row['extension_code'] == 'package_buy') {
			...
			if (($row['parent_id'] == 0) && ($row['is_gift'] == 0)) {
				...
				$sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('cart') . ' WHERE ' . $sess_id . 'AND (rec_id IN (' . $_del_str . ') OR parent_id = \'' . $row['goods_id'] . '\' OR is_gift <> 0) AND group_id=\'' . $row['group_id'] . '\'';
			}
			else {
				$sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('cart') . ' WHERE ' . $sess_id . 'AND rec_id = \'' . $id . '\' LIMIT 1';
			}
...
function get_guess_goods($user_id, $history = 0, $page = 1, $limit = 5, $warehouse_id = 0, $area_id = 0)
{
	...
	if (empty($history)) {
		...
		if ($order_arr) {
			...
			$sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, ' . 'IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * \'' . $_SESSION['discount'] . '\') AS shop_price, ' . 'IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)) as promote_price,' . ' g.sales_volume, g.promote_start_date, g.promote_end_date FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . $leftJoin . 'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' . 'ON mp.goods_id = g.goods_id AND mp.user_rank = \'' . $_SESSION['user_rank'] . '\' ' . ' WHERE ' . $link_cats_str . ' AND g.goods_id NOT IN (' . $finished_goodsStr . ') AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ORDER BY g.sales_volume DESC  LIMIT 8';
		...
		$sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, ' . 'IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * \'' . $_SESSION['discount'] . '\') AS shop_price, ' . 'IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)) as promote_price,' . ' g.sales_volume, g.user_id, g.promote_start_date, g.promote_end_date FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . $leftJoin . 'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' . 'ON mp.goods_id = g.goods_id AND mp.user_rank = \'' . $_SESSION['user_rank'] . '\' ' . ' WHERE ' . $where_cat . ' ' . $goods_str . ' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.sales_volume > 0 LIMIT ' . $start . ', ' . $limit;
		$query = $GLOBALS['db']->query($sql);
	}

	if ((empty($guess_goods) || (count($guess_goods) < $limit)) && ($history == 1)) {
		$guess_goods = array();
		$sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.user_id, ' . 'IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * \'' . $_SESSION['discount'] . '\') AS shop_price, ' . 'IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)) as promote_price,' . 'g.sales_volume, g.promote_start_date, g.promote_end_date FROM ' . $GLOBALS['ecs']->table('goods') . 'as g' . $leftJoin . 'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' . 'ON mp.goods_id = g.goods_id AND mp.user_rank = \'' . $_SESSION['user_rank'] . '\' ' . ' WHERE  (g.sales_volume > 0 OR g.is_hot = 1) AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ORDER BY g.sales_volume DESC LIMIT ' . $start . ', ' . $limit;
		$query = $GLOBALS['db']->query($sql);
	}
...
function get_category_store_list($ru_id, $is_url = 0, $level = 0)
{
	...
	$sql = 'SELECT cat_id, cat_name, parent_id, user_id AS ru_id, sort_order, ' . 'measure_unit, grade, is_show, show_in_nav FROM ' . $GLOBALS['ecs']->table('merchants_category') . ' WHERE ' . $where . ' AND parent_id = 0 ORDER BY sort_order, cat_id DESC';
	$res = $GLOBALS['db']->getAll($sql);
