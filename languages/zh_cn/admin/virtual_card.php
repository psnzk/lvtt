...
$_LANG['use_help'] = '<h4>使用说明：</h4>' . '<ul>' . '<li>上传文件应为CSV文件<br />' . 'CSV文件第一列为卡片序号；第二列为卡片密码；第三列为使用截至日期。<br />' . '(用EXCEL创建csv文件方法：在EXCEL中按卡号、卡片密码、截至日期的顺序填写数据，完成后直接保存为csv文件即可)' . '<li>密码，和截至日期可以为空，截至日期格式为2006-11-6或2006/11/6' . '<li>卡号、卡片密码、截至日期中不要使用中文</li>' . '</ul>';
