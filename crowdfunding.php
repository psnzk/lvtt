...
if ($_REQUEST['act'] == 'post_topic') {
	...
	if (0 < $_SESSION['user_id']) {
		if (!empty($topic_content)) {
			$sql = ' insert into ' . $GLOBALS['ecs']->table('zc_topic') . ' (topic_id, parent_topic_id, reply_topic_id, topic_status, topic_content, user_id, pid, add_time) ' . ' VALUES ' . ' (NULL, \'' . $topic_id . '\', \'' . $parent_id . '\', 1, \'' . $topic_content . '\', \'' . $_SESSION['user_id'] . '\', \'' . $zcid . '\', \'' . time() . '\') ';
...
if ($_REQUEST['act'] == 'submit_topic') {
	...
	if (0 < $_SESSION['user_id']) {
		if (!empty($topic_content)) {
			$sql = ' insert into ' . $GLOBALS['ecs']->table('zc_topic') . ' (topic_id, parent_topic_id, topic_status, topic_content, user_id, pid, add_time) ' . ' VALUES ' . ' (NULL, 0, 1, \'' . $topic_content . '\', \'' . $_SESSION['user_id'] . '\', \'' . $zcid . '\', \'' . time() . '\') ';
		...
		if ($type == 1) {
			if (empty($_SESSION['user_id'])) {
				...
				if (empty($focus_status)) {
					$sql = ' insert into ' . $GLOBALS['ecs']->table('zc_focus') . ' (rec_id,user_id,pid,add_time) ' . ' VALUES ' . ' (NULL, \'' . $_SESSION['user_id'] . '\', \'' . $zcid . '\', \'' . time() . '\') ';
		...
		if ($v['is_online'] == 1) {
			...
			if (file_exists($payment_file)) {
				...
				if ($v['pay_code'] == 'balance') {
					$pay_online_button['balance'] = '		<a href="flow.php?step=done&act=balance&order_sn=' . $order['order_sn'] . '" id="balance" style="float: left;" order_sn="' . $order['order_sn'] . '" flag="balance" >余额支付</a>';
				}
				...
				if (!empty($user_baitao_amount)) {
					...
					if ($v['pay_code'] == 'chunsejinrong') {
						$pay_online_button['chunsejinrong'] = '				<a href="flow.php?step=done&act=chunsejinrong&order_sn=' . $order['order_sn'] . '" id="chunsejinrong" style="float: left;" order_sn="' . $order['order_sn'] . '" flag="chunsejinrong" >白条支付</a>';
					}
				}
			}
		}
