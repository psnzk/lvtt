...
function get_cat_level($parent_id = 0, $level = 0, $ru_id = 0)
{
	...
	if ($level < 3) {
		$sql = 'SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, ' . '(SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('merchants_category') . ' AS mc WHERE mc.cat_id = c.cat_id) AS seller_cat ' . ' FROM ' . $GLOBALS['ecs']->table('category') . ' AS c WHERE c.parent_id = \'' . $parent_id . '\' HAVING seller_cat = 0 ' . ' order by c.sort_order, c.cat_id';
		...
		$sql = 'SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order ' . ' FROM ' . $GLOBALS['ecs']->table('category') . ' AS c, ' . $GLOBALS['ecs']->table('merchants_category') . ' AS mc1 WHERE c.parent_id = \'' . $parent_id . '\' AND (SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('merchants_category') . ' AS mc2 WHERE mc2.cat_id = c.cat_id) = 0 ' . $where . ' ' . ' order by c.sort_order, c.cat_id';
		$res = $GLOBALS['db']->getAll($sql);
	}
