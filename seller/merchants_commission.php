...
function merchants_commission_list()
{
	...
	for ($i = 0; $i < $count; $i++) {
		...
		$valid = get_nerchants_order_valid_refund($row[$i]['user_id']);
		$row[$i]['order_valid_total'] = price_format($valid['total_fee']);
		$refund = get_nerchants_order_valid_refund($row[$i]['user_id'], 1);
...
function merchants_order_list()
{
	...
	for ($i = 0; $i < $count; $i++) {
		...
		$percent_id = get_table_date('merchants_server', 'user_id = \'' . $row[$i]['ru_id'] . '\' ', $date, 2);
