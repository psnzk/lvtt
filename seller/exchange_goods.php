...
if ($_REQUEST['act'] == 'insert') {
	...
	$adminru = get_admin_ru_id();
	$sql = 'INSERT INTO ' . $ecs->table('exchange_goods') . '(goods_id, exchange_integral, is_exchange, is_hot, is_best, user_id) ' . 'VALUES (\'' . $_POST['goods_id'] . '\', \'' . $_POST['exchange_integral'] . '\', \'' . $_POST['is_exchange'] . '\', \'' . $_POST['is_hot'] . '\', \'' . $_POST['is_best'] . '\', \'' . $adminru['ru_id'] . '\')';
	$db->query($sql);
...
if ($_REQUEST['act'] == 'edit') {
	admin_priv('exchange_goods');
	$smarty->assign('menu_select', array('action' => '03_promotion', 'current' => '15_exchange_goods'));
	$sql = 'SELECT eg.goods_id, eg.exchange_integral,eg.is_exchange, eg.is_hot, eg.is_best, g.goods_name ' . ' FROM ' . $ecs->table('exchange_goods') . ' AS eg ' . '  LEFT JOIN ' . $ecs->table('goods') . ' AS g ON g.goods_id = eg.goods_id ' . ' WHERE eg.goods_id=\'' . $_REQUEST['id'] . '\'';
	$goods = $db->GetRow($sql);
