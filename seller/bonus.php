...
if ($_REQUEST['act'] == 'insert') {
	...
	$sql = 'INSERT INTO ' . $ecs->table('bonus_type') . " (type_name, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type, usebonus_type, user_id, min_amount, max_amount, min_goods_amount)\r\n    VALUES ('" . $type_name . "',\r\n            '" . $_POST['type_money'] . "',\r\n            '" . $send_startdate . "',\r\n            '" . $send_enddate . "',\r\n            '" . $use_startdate . "',\r\n            '" . $use_enddate . "',\r\n            '" . $_POST['send_type'] . "',\r\n            '" . $usebonus_type . "',    \r\n            '" . $adminru['ru_id'] . "',\r\n            '" . $min_amount . "',\r\n            '" . $max_amount . '\',' . '\'' . floatval($_POST['min_goods_amount']) . '\'' . ')';
	$db->query($sql);
...
if ($_REQUEST['act'] == 'update') {
	...
	$sql = 'UPDATE ' . $ecs->table('bonus_type') . ' SET ' . 'type_name       = \'' . $type_name . '\', ' . 'type_money      = \'' . $_POST['type_money'] . '\', ' . 'send_start_date = \'' . $send_startdate . '\', ' . 'send_end_date   = \'' . $send_enddate . '\', ' . 'use_start_date  = \'' . $use_startdate . '\', ' . 'use_end_date    = \'' . $use_enddate . '\', ' . 'send_type       = \'' . $_POST['send_type'] . '\', ' . 'usebonus_type   = \'' . $usebonus_type . '\', ' . 'min_amount      = \'' . $min_amount . '\', ' . 'max_amount      = \'' . $max_amount . '\', ' . 'min_goods_amount = \'' . floatval($_POST['min_goods_amount']) . '\' ' . 'WHERE type_id   = \'' . $type_id . '\'';
	$db->query($sql);
...
if ($_REQUEST['act'] == 'query_bonus') {
	...
	$sort_flag = sort_flag($list['filter']);
	$smarty->assign($sort_flag['tag'], $sort_flag['img']);
	make_json_result($smarty->fetch('bonus_list.htm'), '', array('filter' => $list['filter'], 'page_count' => $list['page_count']));
}
