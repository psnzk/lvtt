...
{
	$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('presale_activity') . ' WHERE goods_id = \'' . $goods_id . '\' ' . ' AND start_time <= ' . gmtime() . ' AND end_time >= ' . gmtime();
...
if ($_REQUEST['act'] == 'list') {
	...
	if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit')) {
		...
		if ($_REQUEST['act'] == 'add') {
			$presale = array('cid' => 0, 'start_time' => date('Y-m-d H:i:s', time() + 86400), 'end_time' => date('Y-m-d H:i:s', time() + (4 * 86400)), 'pay_start_time' => date('Y-m-d H:i:s', time() + (4 * 86400) + 1), 'pay_end_time' => date('Y-m-d H:i:s', time() + (6 * 86400)));
		}
		else {
		...
		if (isset($_POST['finish'])) {
			...
			$adminru = get_admin_ru_id();
			$presale = array('act_name' => $act_name, 'act_desc' => $_POST['act_desc'], 'cid' => intval($_POST['cid']), 'goods_id' => $goods_id, 'user_id' => $adminru['ru_id'], 'goods_name' => $goods_name, 'start_time' => $start_time, 'end_time' => $end_time, 'pay_start_time' => $pay_start_time, 'pay_end_time' => $pay_end_time, 'deposit' => $deposit);
			clear_cache_files();
