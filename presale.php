...
function get_pre_goods($cat_id, $min = 0, $max = 0, $start_time = 0, $end_time = 0, $sort, $status = 0, $order)
{
	...
	$sql = 'SELECT a.*, g.goods_thumb, g.goods_img, g.goods_name, g.shop_price, g.market_price, g.sales_volume FROM ' . $GLOBALS['ecs']->table('presale_activity') . ' AS a ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON a.goods_id = g.goods_id ' . ' WHERE g.goods_id > 0 ' . $where . ' AND g.is_on_sale = 0 ORDER BY ' . $sort . ' ' . $order;
...
{
	$now = gmtime();
	$sql = 'SELECT a.*, g.goods_thumb, g.goods_img, g.goods_name, g.shop_price, g.market_price, g.sales_volume FROM ' . $GLOBALS['ecs']->table('presale_activity') . ' AS a ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON a.goods_id = g.goods_id ' . 'WHERE a.cat_id = \'' . $cat_id . '\' AND g.is_on_sale = 0 ';
	$res = $GLOBALS['db']->getAll($sql);
...
function get_presale_time($goods_id)
{
	...
	if ($res['end_time']) {
		...
		if ($res['end_time']) {
			...
			$atthe = explode(':', $end_time[1]);
			$res['str_time'] = $end_time[0] . ' ' . $atthe[0] . ':' . $atthe[1];
		}
		else {
			$res['str_time'] = $res['end_time'];
		}
	}
