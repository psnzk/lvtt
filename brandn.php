...
function brand_recommend_goods($type, $brand, $cat = 0, $warehouse_id = 0, $area_id = 0, $act = '', $type_rand = '')
{
	...
	if ($result === NULL) {
		...
		if ($type_rand == 'rand') {
			$cat_where .= ' AND g.goods_id >= (SELECT floor(RAND() * (SELECT MAX(goods_id) FROM ' . $GLOBALS['ecs']->table('goods') . ' WHERE brand_id=' . $brand . '' . ' AND is_best = 1)))';
		}
		else if ($type == 'best') {
