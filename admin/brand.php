...
if ($_REQUEST['act'] == 'list') {
	...
	$sql = 'INSERT INTO ' . $ecs->table('brand') . '(brand_name, brand_letter, site_url, brand_desc, brand_logo, is_show, sort_order) ' . 'VALUES (\'' . $_POST['brand_name'] . '\', \'' . $_POST['brand_letter'] . '\', \'' . $site_url . '\', \'' . $_POST['brand_desc'] . '\', \'' . $img_name . '\', \'' . $is_show . '\', \'' . $_POST['sort_order'] . '\')';
	$db->query($sql);
	...
	admin_priv('brand_manage');
	$sql = 'SELECT b.brand_id, b.brand_name, b.brand_letter, b.site_url, b.brand_logo, b.brand_desc, b.brand_logo, b.is_show, b.sort_order,be.is_recommend ' . 'FROM ' . $ecs->table('brand') . ' as b left join ' . $ecs->table('brand_extend') . ' as be on b.brand_id=be.brand_id WHERE b.brand_id=\'' . $_REQUEST['id'] . '\'';
	$brand = $db->GetRow($sql);
