...
function get_packagelist($ru_id)
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT ga.act_id, ga.act_name AS package_name, ga.start_time, ga.end_time, ga.is_finished, ga.ext_info, ga.user_id, ga.goods_id ' . ' FROM ' . $GLOBALS['ecs']->table('goods_activity') . ' AS ga ' . ' WHERE ga.act_type = ' . GAT_PACKAGE . $where . ' ORDER by ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' LIMIT ' . $filter['start'] . ', ' . $filter['page_size'];
...
if ($_REQUEST['act'] == 'add') {
	...
	$sql = 'SELECT COUNT(*) ' . ' FROM ' . $ecs->table('goods_activity') . ' WHERE act_type=\'' . GAT_PACKAGE . '\' AND act_name=\'' . $_POST['package_name'] . '\' AND act_id <> \'' . $_POST['id'] . '\'';
