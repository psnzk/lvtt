...
function goods_type_list($selected, $goods_id = 0, $type = 'html')
{
	...
	if ($type == 'array') {
		...
		while ($row = $GLOBALS['db']->fetchRow($res)) {
			$lst[] = array('cat_id' => $row['cat_id'], 'cat_name' => htmlspecialchars($row['cat_name']), 'selected' => $selected == $row['cat_id'] ? 1 : 0);
		}
	}
	else {
		$lst = '';
	...
	if ($result === false) {
		...
		if ($is_ajax == 1) {
			if ($filter['order_id_list']) {
				$sql = 'UPDATE ' . $GLOBALS['ecs']->table('order_info') . ' SET `order_status` = \'' . OS_SPLITED . '\', `shipping_status` = \'' . SS_RECEIVED . '\',`pay_status` = \'' . PS_PAYED . '\' ' . ' WHERE shipping_status = 1 AND order_id IN(' . $filter['order_id_list'] . ')';
				$GLOBALS['db']->query($sql);
			}
...
{
	$sql = ' SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values ' . ' FROM ' . $GLOBALS['ecs']->table('attribute') . ' AS a ' . ' WHERE a.cat_id = ' . intval($goods_type) . ' OR a.cat_id = 0 ' . ' ORDER BY a.sort_order, a.attr_type, a.attr_id ';
	$attribute_list = $GLOBALS['db']->getAll($sql);
	$sql = ' SELECT v.attr_id, v.attr_value, v.attr_price, v.attr_sort, v.attr_checked ' . ' FROM ' . $GLOBALS['ecs']->table('goods_attr') . ' AS v ' . ' WHERE goods_id = \'' . $goods_id . '\' ';
...
{
	$sql = 'SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order ' . ' FROM ' . $GLOBALS['ecs']->table($table) . ' AS c WHERE c.parent_id = \'' . $parent_id . '\' ' . ' order by c.sort_order, c.cat_id';
	...
	foreach ($res as $k => $row) {
		if ($table == 'category') {
			...
			$goodsNums = $GLOBALS['db']->getAll('SELECT * FROM ' . $GLOBALS['ecs']->table($goods_table) . ' AS g ' . ' WHERE g.is_delete = 0 ' . $cat_in . $ruCat);
			$goods_ids = array();
