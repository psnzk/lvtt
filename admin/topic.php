...
$topic_style_color = array('0' => '008080', '1' => '008000', '2' => 'ffa500', '3' => 'ff0000', '4' => 'ffff00', '5' => '9acd32', '6' => 'ffd700');
...
if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit')) {
	...
	if (($_REQUEST['act'] == 'insert') || ($_REQUEST['act'] == 'update')) {
		...
		if ($is_insert) {
			$adminru = get_admin_ru_id();
			$sql = 'INSERT INTO ' . $ecs->table('topic') . ' (title,start_time,end_time,data,intro,template,css,topic_img,title_pic,base_style, htmls,keywords,description, user_id)' . 'VALUES (\'' . $_POST['topic_name'] . '\',\'' . $start_time . '\',\'' . $end_time . '\',\'' . $data . '\',\'' . $_POST['topic_intro'] . '\',\'' . $_POST['topic_template_file'] . '\',\'' . $_POST['topic_css'] . '\', \'' . $topic_img . '\', \'' . $title_pic . '\', \'' . $base_style . '\', \'' . $htmls . '\',\'' . $keywords . '\',\'' . $description . '\', \'' . $adminru['ru_id'] . '\')';
		}
		else {
			$insert_data = '';

			if ($topic_img) {
				$insert_data .= 'topic_img=\'' . $topic_img . '\',';
			}

			if ($title_pic) {
				$insert_data .= 'title_pic=\'' . $title_pic . '\',';
			}

			$sql = 'UPDATE ' . $ecs->table('topic') . 'SET title=\'' . $_POST['topic_name'] . '\',start_time=\'' . $start_time . '\',end_time=\'' . $end_time . '\',data=\'' . $data . '\',intro=\'' . $_POST['topic_intro'] . '\',template=\'' . $_POST['topic_template_file'] . '\',css=\'' . $_POST['topic_css'] . '\', ' . $insert_data . ' base_style=\'' . $base_style . '\', htmls=\'' . $htmls . '\', keywords=\'' . $keywords . '\', description=\'' . $description . '\'' . ' WHERE topic_id=\'' . $topic_id . '\' LIMIT 1';
		}
