...
function order_download_list($result)
{
	...
	for ($i = 0; $i < $count; $i++) {
		...
		$data .= $order_sn . ',' . $short_order_time . ',' . $consignee . ',' . $total_fee_price . ',' . $return_amount_price . ',' . $brokerage_amount_price . ',' . $status . ',' . $effective_amount_price . ',' . $is_settlement . "\n";
	}
...
function merchants_order_list()
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT o.order_id ' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' AS u ON u.user_id=o.user_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('order_goods') . ' AS og ON o.order_id=og.order_id ' . $where;
		...
		$sql = 'SELECT ' . $where_select . ' og.ru_id, o.order_id, o.main_order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.is_delete, o.is_settlement,' . 'o.shipping_time, o.auto_delivery_time, o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id, ' . '(' . order_commission_field('o.') . ') AS total_fee, ' . 'IFNULL(u.user_name, \'' . $GLOBALS['_LANG']['anonymous'] . '\') AS buyer ' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' AS u ON u.user_id=o.user_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('order_goods') . ' AS og ON o.order_id=og.order_id ' . $where . ' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' ';
		$sql .= ' LIMIT ' . (($filter['page'] - 1) * $filter['page_size']) . ', ' . $filter['page_size'] . ' ';
		set_filter($filter, $sql);
	}
	else {
	...
	if ($count) {
		$row['brokerage_amount']['all'] = price_format($row['all_brokerage_amount'], true);
		...
		unset($row['all_brokerage_amount']);
		$is_settlement = merchants_is_settlement($row['brokerage_amount']['ru_id'], 1);
		$no_settlement = merchants_is_settlement($row['brokerage_amount']['ru_id'], 0);
...
function get_merchants_order_valid_refund($ru_id, $type = 0)
{
	...
	if ($type == 1) {
		...
		$sql = 'SELECT oi.order_id, oi.order_sn, ' . $total_fee . ' ' . $where_select . '  FROM ' . $GLOBALS['ecs']->table('order_info') . ' as oi ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('order_goods') . ' AS og2 ON og2.order_id = oi.order_id ' . ' WHERE 1 ' . $where . ' AND (SELECT og.ru_id FROM ' . $GLOBALS['ecs']->table('order_goods') . ' as og' . ' WHERE og.order_id = oi.order_id limit 0, 1) = \'' . $ru_id . '\'' . ' AND (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = oi.order_id limit 0, 1) = 0 LIMIT 1';
		$res = $GLOBALS['db']->getRow($sql);
	}
