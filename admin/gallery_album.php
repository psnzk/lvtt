...
if ($_REQUEST['act'] == 'list') {
	...
	if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit')) {
		...
		if (($_REQUEST['act'] == 'insert') || ($_REQUEST['act'] == 'update')) {
			...
			$sql = 'SELECT album_id,album_mame FROM ' . $GLOBALS['ecs']->table('gallery_album');
			...
			$sql = 'SELECT pic_file,pic_thumb FROM' . $ecs->table('pic_album') . ' WHERE album_id = ' . $id;
			...
			if (!empty($checkboxes)) {
				...
				$sql = 'SELECT pic_file,pic_thumb FROM' . $ecs->table('pic_album') . ' WHERE album_id' . db_create_in($checkboxes);
				$pic_info = $db->getAll($sql);
			...
			if (!empty($checkboxes)) {
				if ($type == 'remove') {
					$sql = 'SELECT pic_file,pic_thumb FROM' . $ecs->table('pic_album') . ' WHERE pic_id' . db_create_in($checkboxes);
					$pic_info = $db->getAll($sql);
