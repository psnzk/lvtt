...
if ($_REQUEST['act'] == 'setup') {
	...
	$cat_articles = array();
	$ad_positions = array();
	$sql = 'SELECT region, library, sort_order, id, number, type FROM ' . $ecs->table('template') . ' ' . 'WHERE theme=\'' . $template_theme . '\' AND filename=\'' . $curr_template . '\' AND remarks=\'\' ' . 'ORDER BY region, sort_order ASC ';
	$rc = $db->query($sql);
	$db_dyna_libs = array();
	...
	if (isset($_POST['regions']['cat_goods'])) {
		foreach ($_POST['regions']['cat_goods'] as $key => $val) {
			if (($_POST['categories']['cat_goods'][$key] != '') && (0 < intval($_POST['categories']['cat_goods'][$key]))) {
				$sql = 'INSERT INTO ' . $ecs->table('template') . ' (' . 'theme, filename, region, library, sort_order, type, id, number' . ') VALUES (' . '\'' . $curr_template . '\', ' . '\'' . $_POST['template_file'] . '\', \'' . $val . '\', \'/library/cat_goods.lbi\', ' . '\'' . $_POST['sort_order']['cat_goods'][$key] . '\', 1, \'' . $_POST['categories']['cat_goods'][$key] . '\', \'' . $_POST['number']['cat_goods'][$key] . '\'' . ')';
				$db->query($sql);
			}
		}
	}
