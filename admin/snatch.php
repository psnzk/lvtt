...
function get_snatchlist($ru_id)
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT ga.act_id, ga.act_name AS snatch_name, ga.goods_name, ga.start_time, ga.end_time, ga.is_finished, ga.ext_info, ga.product_id, ga.user_id, ga.is_hot ' . ' FROM ' . $GLOBALS['ecs']->table('goods_activity') . ' AS ga ' . ' WHERE ga.act_type = ' . GAT_SNATCH . $where . ' ORDER by ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' LIMIT ' . $filter['start'] . ', ' . $filter['page_size'];
...
function get_snatch_info($id)
{
	...
	global $db;
	global $_CFG;
	$sql = 'SELECT act_id, act_name AS snatch_name, goods_id, product_id, goods_name, start_time, end_time, act_desc, act_promise, act_ensure, ext_info, is_hot' . ' FROM ' . $GLOBALS['ecs']->table('goods_activity') . ' WHERE act_id=\'' . $id . '\' AND act_type = ' . GAT_SNATCH;
...
if ($_REQUEST['act'] == 'add') {
	...
	$record = array('act_name' => $_POST['snatch_name'], 'act_desc' => $_POST['desc'], 'act_type' => GAT_SNATCH, 'goods_id' => $_POST['activity_goods'], 'goods_name' => $_POST['goods_name'], 'start_time' => $_POST['start_time'], 'end_time' => $_POST['end_time'], 'product_id' => $_POST['product_id'], 'is_hot' => $_POST['is_hot'], 'user_id' => $adminru['ru_id'], 'act_desc' => $act_desc, 'act_promise' => $act_promise, 'act_ensure' => $dact_ensure, 'is_finished' => 0, 'ext_info' => serialize($info));
	...
	$sql = 'SELECT COUNT(*) ' . ' FROM ' . $ecs->table('goods_activity') . ' WHERE act_type=\'' . GAT_SNATCH . '\' AND act_name=\'' . $_POST['snatch_name'] . '\' AND act_id <> \'' . $_POST['id'] . '\'';
	...
	$info = array('start_price' => $_POST['start_price'], 'end_price' => $_POST['end_price'], 'max_price' => $_POST['max_price'], 'cost_points' => $_POST['cost_points']);
	$record = array('act_name' => $_POST['snatch_name'], 'goods_id' => $_POST['activity_goods'], 'goods_name' => $_POST['goods_name'], 'start_time' => $_POST['start_time'], 'end_time' => $_POST['end_time'], 'act_desc' => $_POST['desc'], 'product_id' => $_POST['product_id'], 'is_hot' => $_POST['is_hot'], 'act_desc' => $act_desc, 'act_promise' => $act_promise, 'act_ensure' => $dact_ensure, 'ext_info' => serialize($info));
