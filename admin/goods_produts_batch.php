...
function get_list_download($goods_sn = '', $warehouse_info = array(), $attr_info, $attr_num)
{
	...
	if ($model_attr == 0) {
		...
		foreach ($res as $k => $v) {
			$arr[] = array('goods_sn' => $goods_sn, 'region_name' => '默认模式', 'attr_value' => $v, 'product_sn' => '', 'product_number' => '');
		}
	}
...
if ($_REQUEST['act'] == 'add') {
	...
	if (isset($_SESSION['goods_list']) && $_SESSION['goods_list']) {
		...
		if ($GLOBALS['db']->getOne($sql)) {
			...
			$other = array('goods_id' => $result['list']['goods_id'], 'goods_attr' => $result['list']['goods_attr'], 'product_sn' => $result['list']['product_sn'], 'product_number' => $result['list']['product_number']);
