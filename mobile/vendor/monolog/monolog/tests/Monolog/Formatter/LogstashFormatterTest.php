...
class LogstashFormatterTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testDefaultFormatter()
	{
		...
		$this->assertEquals(\Monolog\Logger::ERROR, $message['@fields']['level']);
	...
	public function testFormatWithLatin9Data()
	{
		...
		$record = array(
			...
			'context'    => array(),
			'datetime'   => new \DateTime('@0'),
			'extra'      => array('user_agent' => 'ÖWN; FBCR/OrangeEspaña; Versão/4.0; Färist'),
			'message'    => 'log'
			);
		$message = json_decode($formatter->format($record), true);
		$this->assertEquals('1970-01-01T00:00:00.000000+00:00', $message['@timestamp']);
		$this->assertEquals('log', $message['@message']);
		$this->assertEquals('Â¯\\_(ãƒ?_/Â¯', $message['@fields']['channel']);
		$this->assertContains('Â¯\\_(ãƒ?_/Â¯', $message['@tags']);
		$this->assertEquals(\Monolog\Logger::ERROR, $message['@fields']['level']);
		$this->assertEquals('test', $message['@type']);
		$this->assertEquals('hostname', $message['@source']);

		if (version_compare(PHP_VERSION, '5.5.0', '>=')) {
			$this->assertEquals('Ã–WN; FBCR/OrangeEspaÃ±a; VersÃ£o/4.0; FÃ¤rist', $message['@fields']['user_agent']);
		}
		else {
