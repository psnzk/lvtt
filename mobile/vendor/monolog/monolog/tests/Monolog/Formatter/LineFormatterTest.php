...
class LineFormatterTest extends \PHPUnit_Framework_TestCase
{
	...
	'datetime'   => new \DateTime(),
	'extra'      => array(),
	'context'    => array('foo' => 'bar', 'baz' => 'qux', 'bool' => false, 'null' => null)
	));
	...
	'message'    => 'foobar'
		...
		$this->assertEquals('[' . date('Y-m-d') . '] core.CRITICAL: foobar {"exception":"[object] (RuntimeException(code: 0): Foo at ' . substr($path, 1, -1) . ':' . (137 - 8) . ')"} []' . "\n", $message);
	}
	...
	'message'    => 'foobar'
		...
		$path = str_replace('\\/', '/', json_encode(__FILE__));
		$this->assertEquals('[' . date('Y-m-d') . '] core.CRITICAL: foobar {"exception":"[object] (RuntimeException(code: 0): Foo at ' . substr($path, 1, -1) . ':' . (155 - 8) . ', LogicException(code: 0): Wut? at ' . substr($path, 1, -1) . ':' . (155 - 12) . ')"} []' . "\n", $message);
	}
