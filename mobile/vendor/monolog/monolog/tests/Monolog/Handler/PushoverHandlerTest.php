...
class PushoverHandlerTest extends \Monolog\TestCase
{
	private $res;
	private $handler;
	...
	{
		$this->createHandler();
		$this->handler->setHighPriorityLevel(\Monolog\Logger::EMERGENCY);
	...
	public function testWriteWithTooLongMessage()
	{
		...
		$this->createHandler();
		$this->handler->setHighPriorityLevel(\Monolog\Logger::EMERGENCY);
		$this->handler->handle($this->getRecord(\Monolog\Logger::CRITICAL, $message));
		fseek($this->res, 0);
		$content = fread($this->res, 1024);
	...
	{
		$this->createHandler();
		$this->handler->handle($this->getRecord(\Monolog\Logger::EMERGENCY, 'test1'));
		fseek($this->res, 0);
		$content = fread($this->res, 1024);
