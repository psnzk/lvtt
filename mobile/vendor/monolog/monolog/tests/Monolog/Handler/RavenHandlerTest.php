...
class RavenHandlerTest extends \Monolog\TestCase
{
	...
	public function testTag()
	{
		...
		$handler = $this->getHandler($ravenClient);
		$tags = array(1, 2, 'foo');
		$record = $this->getRecord(\Monolog\Logger::INFO, 'test', array('tags' => $tags));
		$handler->handle($record);
	...
	public function testExtraParameters()
	{
		...
		$record = $this->getRecord(\Monolog\Logger::INFO, 'test', array('checksum' => $checksum, 'release' => $release));
		$handler->handle($record);
	...
	public function testFingerprint()
	{
		...
		$record = $this->getRecord(\Monolog\Logger::INFO, 'test', array('fingerprint' => $fingerprint));
		$handler->handle($record);
	...
	public function testUserContext()
	{
		$ravenClient = $this->getRavenClient();
		$handler = $this->getHandler($ravenClient);
		$recordWithNoContext = $this->getRecord(\Monolog\Logger::INFO, 'test with default user context');
		$user = array('id' => '123', 'email' => 'test@test.com');
		$recordWithContext = $this->getRecord(\Monolog\Logger::INFO, 'test', array('user' => $user));
	...
	public function testException()
	{
		...
		try {
			...
			$record = $this->getRecord(\Monolog\Logger::ERROR, $e->getMessage(), array('exception' => $e));
			$handler->handle($record);
		}
	...
	public function testHandleBatch()
	{
		...
		$records[] = $this->getRecord(\Monolog\Logger::WARNING, 'warning');
		$records[] = $this->getRecord(\Monolog\Logger::WARNING, 'warning');
	...
	public function testGetSetBatchFormatter()
	{
		$ravenClient = $this->getRavenClient();
		$handler = $this->getHandler($ravenClient);
		$handler->setBatchFormatter($formatter = new \Monolog\Formatter\LineFormatter());
	...
	public function testRelease()
	{
		...
		$record = $this->getRecord(\Monolog\Logger::INFO, 'test');
		$handler->handle($record);
		$this->assertEquals($release, $ravenClient->lastData['release']);
		$localRelease = 'v41.41.41';
		$record = $this->getRecord(\Monolog\Logger::INFO, 'test', array('release' => $localRelease));
		$handler->handle($record);
