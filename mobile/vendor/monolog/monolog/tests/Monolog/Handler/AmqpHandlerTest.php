...
class AmqpHandlerTest extends \Monolog\TestCase
{
	public function testHandleAmqpExt()
	{
		...
		$expected = array(
			...
			'warn.test',
			0,
			array('delivery_mode' => 2, 'content_type' => 'application/json')
			);
	...
	public function testHandlePhpAmqpLib()
	{
		...
		$exchange->expects($this->any())->method('basic_publish')->will($this->returnCallback(function(\PhpAmqpLib\Message\AMQPMessage $msg, $exchange = '', $routing_key = '', $mandatory = false, $immediate = false, $ticket = NULL) use(&$messages) {
