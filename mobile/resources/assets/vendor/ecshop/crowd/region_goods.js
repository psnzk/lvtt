...
region.changedDis = function(district_id, city_id, user_id, d_null){
    ...
    var url = ROOT_URL + 'index.php?m=goods&c=index&a=in_stock';
      ...
      if (confirm(res.message))
        {
          ...
          location.href = ROOT_URL + 'index.php?m=user&c=index&a=address_list';
        }
        else{
...
function warehouse(region_id, goodsId){
    if(region_id && goodsId){
        var url = ROOT_URL + 'index.php?m=goods&c=index&a=in_warehouse';
...
{
    $.get(ROOT_URL + 'index.php?m=region&c=index&a=select_region_child', {raId:raId, parent:parent, type:type}, function(data){
        if(data.regions.length > 0){
            var str = "";
            var regions = data.regions;
            for(key in regions){
...
region.selectDis = function(region_id, type){
    $.get(ROOT_URL + 'index.php?m=region&c=index&a=select_district_list', {region_id: region_id, type: type}, function(data){
        if(data.error == 0){
            location.reload();
        }
    }, 'json');
...
region.cccDdd = function(district_id, city_id, user_id, d_null){
    ...
    var province_id = document.getElementById('province_id').value;
    var url = ROOT_URL + 'index.php?m=user&c=index&a=addaddress/in_stock';
    if( city_id <= 0){
        return false;
    }
    if(d_null == 1){
   ...
   $.post(ROOT_URL + 'index.php?m=user&c=index&a=show_region_name',{province:province,city:city,district:district},function(obj){
