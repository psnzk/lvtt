...
function addToCart(goodsId, parentId) {
    ...
    $.post(ROOT_URL + 'index.php?m=cart&c=index&a=add_to_cart', {
        goods: $.toJSON(goods)
    }, function(data) {
    ...
    if (result.error > 0) {
        ...
        if (result.error == 2)
        {
            layer.open({
                ...
                shadeClose: false,
                yes: function() {
                    location.href = ROOT_URL + 'index.php?m=user&c=index&a=add_booking&id=' + result.goods_id + '&spec=' + result.product_spec;
                },
                no: function() {
                }
            });
        ...
        // 没选规格，弹出属性选择框
            ...
            location.href = ROOT_URL + 'index.php?m=goods&c=index&a=index&id='+result.goods_id;
        ...
        var cart_url = ROOT_URL + 'index.php?m=cart&c=index&a=index';
...
function collect(goodsId) {
    $.get(ROOT_URL + 'index.php?m=goods&c=index&a=add_collection', {
        id: goodsId
    }, function(data) {
...
function addPackageToCart(packageId, areaId, regionId) {
    ...
    $.post(ROOT_URL + 'index.php?m=flow&c=index&a=add_package_to_cart', {
    ...
    if (result.error > 0) {
        if (result.error == 2) {
            layer.open({
                ...
                shadeClose: false,
                yes: function () {
                    window.location.href = ROOT_URL + 'index.php?m=user&c=index&a=add_booking&id=' + result.package_id;
                },
                no: function () {
                }
            });
...
function in_addToCart(goodsId, parentId)
{
  ...
  goods.number   = number;
  goods.parent   = (typeof(parentId) == "undefined") ? 0 : parseInt(parentId);
  $.ajax({ url: ROOT_URL + "index.php?m=cart&c=index&a=add_to_cart",type:"POST",data:{goods: $.toJSON(goods)}, dataType:"json", success: function(data){
          if(data.error==0){
        	  d_messages('已加入购物车', 2);
          }else{
