function register_sms(){
	...
	$.post(ROOT_URL + "index.php?m=user&c=index&a=register_sms", { "mobile": mobile, "sms_code": code },function(result){
		alert(result);
	}, "json");
...
function sendSms(){
	...
	RemainTime();
	$.post(ROOT_URL + "index.php?m=sms&c=index&a=send", { "mobile": mobile, "sms_code": code },function(result){
		if (result.code==2){
			alert('验证码已成功发送到您的手机');
		}else{
			if(result.msg){
