...
$(function ($) {
    ...
    $(".flow-receipt .r-btn-submit").click(function () {
        if ($("body").hasClass("show-receipt-div")) {
            ...
            if (f_r_title && f_r_cont != '不开发票') {
                $.get(ROOT_URL + "index.php?m=flow&c=index&a=change_needinv", {
                    need_inv: 1,
                    inv_type: '',
                    inv_payee: f_r_title,
    ...
    $(".flow-coupon .c-btn-submit").click(function () {
        if ($("body").hasClass("show-coupon-div")) {
            ...
            $.get(ROOT_URL + "index.php?m=flow&c=index&a=change_bonus", {
                bonus: bonus
            }, function (result) {
                if (result.error) {
    ...
    $(".flow-coupon .cou-btn-submit").click(function () {
        if ($("body").hasClass("show-coupon-div-1")) {
            ...
            $.get(ROOT_URL + "index.php?m=flow&c=index&a=change_coupont", {
                cou_id: cou_id
            }, function (result) {
                if (result.error) {
                    d_messages(obj.error);
...
function receivebonus(id) {
    $.ajax({
        type: "GET",
        url: ROOT_URL + "index.php?m=coupont&c=index&a=getcoupon",
        data: {
            cou_id: id
        },
...
function getAddress() {
    ...
    function callBackFn(option) {
        ...
        if (document.getElementById("goods_id")) {
            ...
            var url = ROOT_URL + 'index.php?m=goods&c=index&a=instock';
