...
{
	$list = array();
	$sql = 'SELECT w.*, g.goods_thumb, g.goods_name as goods_name ' . $countSql . ' FROM ' . $GLOBALS['ecs']->table('wholesale') . ' AS w, ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . $where . ' AND w.goods_id = g.goods_id ' . $where_sort;
