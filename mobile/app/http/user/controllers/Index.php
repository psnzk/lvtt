...
class Index extends \app\http\base\controllers\Frontend
{
	...
	public function actionHistory()
	{
		if (IS_AJAX) {
			...
			exit(json_encode(array('history' => $history['goods_list'], 'totalPage' => $page_size)));
		}
	...
	public function actionAddComment()
	{
		if (IS_POST) {
			...
			$condition = array('order_id' => $order_id, 'id_value' => $goods_id, 'user_id' => $_SESSION['user_id']);
		...
		$sql = 'SELECT COUNT(*)as num FROM {pre}comment_img where order_id = \'' . $order_id . '\'';
	...
	public function actionAffiliate()
	{
		...
		$str = url('/', '', true, true) . '?u=' . $this->user_id;
		...
		if (!file_exists($filename)) {
			$code = \ectouch\QRcode::png($str, $filename, $errorCorrectionLevel, $matrixPointSize, 2);
