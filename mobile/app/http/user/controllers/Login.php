...
class Login extends \app\http\base\controllers\Frontend
{
	...
	public function actionIndex()
	{
		if (IS_POST) {
			...
			$back_act = (empty($back_act) ? url('/user') : $back_act);
		...
		if (empty($back_act)) {
			if (empty($back_act) && isset($GLOBALS['_SERVER']['HTTP_REFERER'])) {
				$back_act = (strpos($GLOBALS['_SERVER']['HTTP_REFERER'], url('/user')) ? url('/user') : $GLOBALS['_SERVER']['HTTP_REFERER']);
			}
			else {
				$back_act = url('/user');
			}
		...
		$oauth_list = $this->model->table('touch_auth')->where($condition)->order('id asc')->select();
	...
	public function actionLogout()
	{
		if ((!isset($this->back_act) || empty($this->back_act)) && isset($_SERVER['HTTP_REFERER'])) {
			$this->back_act = stripos($_SERVER['HTTP_REFERER'], 'user/profile') ? url('user/index/index') : $_SERVER['HTTP_REFERER'];
		}
		else {
	...
	{
		if (IS_POST) {
			$back_act = (isset($_POST['back_act']) ? trim($_POST['back_act']) : url('/user'));
