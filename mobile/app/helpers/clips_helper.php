...
function get_account_log($user_id, $num, $start, $id)
{
	...
	if (!empty($id)) {
		$sql = 'SELECT ua.*, uaf.bank_number FROM ' . $GLOBALS['ecs']->table('user_account') . ' AS ua' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('user_account_fields') . ' AS uaf ' . ' ON ua.user_id = uaf.user_id ' . ' WHERE ua.user_id = \'' . $user_id . '\' AND ua.id=\'' . $id . '\'';
		$res = $GLOBALS['db']->query($sql);
	}
	else {
		$sql = 'SELECT ua.*, uaf.bank_number FROM ' . $GLOBALS['ecs']->table('user_account') . ' AS ua ' . ' LEFT JOIN  ' . $GLOBALS['ecs']->table('user_account_fields') . ' AS uaf ' . ' ON  ua.user_id = uaf.user_id ' . ' WHERE ua.user_id = \'' . $user_id . '\'' . ' AND process_type in (' . implode(',', array(SURPLUS_SAVE, SURPLUS_RETURN)) . ')  ORDER BY add_time DESC';
		$res = $GLOBALS['db']->selectLimit($sql, $num, $start);
	}
