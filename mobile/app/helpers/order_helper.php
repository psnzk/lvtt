...
function order_goods($order_id)
{
	...
	foreach ($res as $row) {
		...
		if ($row['extension_code'] == 'package_buy') {
			...
			$sql = 'SELECT activity_thumb FROM {pre}goods_activity WHERE act_id =' . $row['goods_id'] . ' and is_finished = 0';
...
function cart_goods($type = CART_GENERAL_GOODS, $cart_value = '', $ru_type = 0, $warehouse_id = 0, $area_id = 0, $consignee = '', $store_id = 0)
{
	...
	foreach ($arr as $key => $value) {
		...
		if ($value['extension_code'] == 'package_buy') {
			...
			foreach ($arr[$key]['package_goods_list'] as $pglk => $pglv) {
				$subtotal += $pglv['rank_price'] * $pglv['goods_number'];
			}
		}

		$arr[$key]['package_list_subtotal'] = $subtotal;
		$arr[$key]['formated_package_list_subtotal'] = price_format($subtotal);
		$arr[$key]['package_saving'] = $arr[$key]['package_list_subtotal'] - $value['goods_price'];
		$arr[$key]['formated_package_saving'] = price_format(abs($arr[$key]['package_saving']));
		...
		if ($value['rec_type'] == 1) {
			...
			$arr[$key]['url'] = url('package/index/index');
		}
		else {
		...
		if ($value['extension_code'] == 'package_buy') {
			$sql = 'SELECT activity_thumb FROM {pre}goods_activity WHERE act_id =' . $value['goods_id'] . ' and is_finished = 0';
			$activity_thumb = $GLOBALS['db']->getRow($sql);
			...
			foreach ($favourable_list as $list) {
				...
				if ($val['user_id'] == $list['user_id']) {
					if (0 <= strpos($list['user_rank'], $_SESSION['user_rank'])) {
						...
						$goods_list_dsh[$key]['act_id'] = $list['act_id'];
					}
				}
			}
...
function get_cart_ru_goods_list($goods_list, $cart_value = '', $consignee = '')
{
	...
	foreach ($goods_list as $key => $row) {
		...
		$arr[$key]['url'] = url('store/index/index', array('id' => $key));
