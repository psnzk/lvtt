...
function get_user_default($user_id = 0)
{
	...
	$info['nick_name'] = $user_nick['nick_name'];
	...
	$info['user_picture'] = get_image_path($user_nick['user_picture']);
	$info['username'] = $user_nick['nick_name'];
	return $info;
}
		...
		foreach ($arr as $warehouse => $row) {
			foreach ($row as $gw => $grow) {
				if (($grow['extension_code'] == 'package_buy') || ($grow['is_shipping'] == 0)) {
					...
					if ($grow['is_real'] == 1) {
						@$new_arr[$key][$warehouse]['number'] += $grow['goods_number'];
					}
