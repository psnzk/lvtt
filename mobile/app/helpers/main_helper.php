...
function get_wechat_user_info($id = 0)
{
	...
	if (is_wechat_browser()) {
		$sql = 'SELECT u.user_name, u.nick_name, wu.headimgurl, wu.nickname ' . ' FROM ' . $GLOBALS['ecs']->table('users') . ' AS u ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('wechat_user') . ' AS wu ' . ' ON  wu.ect_uid = u.user_id ' . ' WHERE u.user_id = \'' . $id . '\' ';
	}

	$result = $GLOBALS['db']->getRow($sql);
	$results['nick_name'] = !empty($result['nickname']) ? $result['nickname'] : (!empty($result['nick_name']) ? $result['nick_name'] : $result['user_name']);
	$results['user_picture'] = !empty($result['headimgurl']) ? $result['headimgurl'] : $result['user_picture'];
	return $results;
}
