...
function get_image_path($image = '', $path = '')
{
	if (strtolower(substr($image, 0, 4)) == 'http') {
		...
		if (config('shop.open_oss') == 1) {
			...
			$url = (empty($image) ? $no_picture : __STATIC__ . '/' . $path . $image);
		}
	}
...
function get_wechat_image_path($image = '')
{
	if (strtolower(substr($image, 0, 11)) == 'app/modules') {
		$url = $image;
	}
	else {
...
{
	$properties = get_goods_properties($goods_id, $warehouse_id, $area_id);
